            ...
            if st.session_state['last_clicked'] == "Admissions":
                ...
                with st.form(key='date_form',border=False):
                    ...
                    with col2:
                        ...
                        # Check if selected dates are within the allowed range
                        if start_date < default_start_date or end_date > default_end_date:
                            st.error(f"Please select a date range between {default_start_date} and {default_end_date}.")
                            start_date, end_date = default_start_date, default_end_date  # Reset to default dates
                    with col3:
                        # Use the second column for the 'N' days input
                        n_days = st.number_input("Choose the historical data duration, in days", min_value=1, value=1)
                    ...
                    old_end_date = default_start_date - timedelta(days=1)
